...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.3cm}p{3.0cm}p{3.1cm}X}
\toprule
技术路线 & 代表对象 & 复杂度主要落点 & 对国产算力适配的关键意义 \\
\midrule
插件式后端适配 & vLLM-Ascend、vLLM-MLU、SGLang Ascend\citep{vllmascend,vllmmlu,sglang_ascend_npu} & plugin backend、attention backend、分布式运行时与版本矩阵 & 以较低上层接口改动把 Ascend NPU、寒武纪 MLU 等国产硬件纳入 vLLM/SGLang 语义，但成熟度受 CANN、torch\_npu、厂商 SDK 与上游版本节奏约束 \\
平台一体化 & MindIE\citep{mindie} & SDK/runtime/compiler、服务模板、监控与交付链路 & 把昇腾平台的编译、运行时和服务化交付收敛到平台栈内部，适合强调统一交付的场景，但不宜写成完全开源的通用引擎 \\
...
跨架构编译/运行时探索 & 中国电信 Triton 统一跨架构框架\citep{chinatelecom_triton_validation_2025,chinatelecom_wbba_2026} & compiler/runtime abstraction、统一算子库、透明运行时插件 & 试图缓解多国产芯片重复适配问题；当前更适合作为跨架构抽象探索，不能写成已开源成熟 serving 引擎 \\
\bottomrule
\end{tabularx}
\end{table*}
